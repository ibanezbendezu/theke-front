...
export interface Diagram { id: string; projectId: string; name: string; document: DiagramDocument; archivedAt: string | null; createdAt: string; updatedAt: string }
...
export function useDiagrams(projectId?: string, status: 'active' | 'archived' = 'active') {
  ...
  return useQuery({ queryKey: ['private', 'diagrams', userId, projectId, status], enabled: Boolean(userId && projectId), queryFn: async ({ signal }) => { const response = await thekeFetch<{ data: Envelope<Diagram[]> }>(`/v1/projects/${projectId}/diagrams?status=${status}`, { headers: await authHeaders(getToken), signal }); return response.data.data; } });
}
