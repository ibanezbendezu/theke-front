...
function ProjectWorkspace({ projectId, openImpact }: { projectId: string; openImpact: (request: ImpactRequest) => void }) {
  ...
  return <div className="mt-6">
    <section aria-labelledby="diagrams-title" className="rounded-lg border border-border p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 id="diagrams-title" className="font-semibold">Diagramas</h2><p className="text-sm text-outline">El lienzo conecta recursos como nodos, grupos y enlaces.</p></div><div className="flex gap-1"><Button variant={diagramStatus === 'active' ? 'secondary' : 'ghost'} onClick={() => setDiagramStatus('active')}>Activos</Button><Button variant={diagramStatus === 'archived' ? 'secondary' : 'ghost'} onClick={() => setDiagramStatus('archived')}>Archivados</Button></div></div>
      {diagramStatus === 'active' && <form className="mt-4 flex gap-2" onSubmit={async event => { event.preventDefault(); if (!diagramName.trim()) return; const created = await diagramActions.create.mutateAsync(diagramName.trim()); setDiagramName(''); navigate(`/projects/${projectId}/diagrams/${created.id}`); }}><Input aria-label="Nombre del diagrama" value={diagramName} maxLength={120} onChange={event => setDiagramName(event.target.value)} /><Button type="submit" variant="primary" disabled={!diagramName.trim() || diagramActions.create.isPending}>Crear diagrama</Button></form>}
      ...
      {!diagramQuery.isPending && !diagramQuery.isError && diagramQuery.data?.length === 0 && <div className="mt-5 rounded border border-dashed border-border p-6 text-center"><Network className="mx-auto text-outline"/><h3 className="mt-2 font-medium">{diagramStatus === 'active' ? 'Aún no hay diagramas' : 'No hay diagramas archivados'}</h3><p className="mt-1 text-sm text-outline">Crea un lienzo para organizar visualmente el contenido de este proyecto.</p></div>}
      {diagramQuery.data?.map(diagram => <div key={diagram.id} className="mt-3 flex items-center gap-2 border-b border-border py-2"><button className="flex min-w-0 flex-1 items-center gap-2 text-left hover:underline" onClick={() => navigate(`/projects/${projectId}/diagrams/${diagram.id}`)}><Network size={18}/><span className="truncate">{diagram.name}</span></button>{diagramStatus === 'active' ? <><Button onClick={() => { const name = window.prompt('Nuevo nombre', diagram.name); if (name?.trim()) diagramActions.rename.mutate({ id: diagram.id, name }); }}>Renombrar</Button><Button size="icon" title={`Duplicar ${diagram.name}`} aria-label={`Duplicar ${diagram.name}`} icon={Copy} onClick={() => diagramActions.duplicate.mutate(diagram.id)}/><Button size="icon" title={`Archivar ${diagram.name}`} aria-label={`Archivar ${diagram.name}`} icon={Archive} onClick={() => openImpact({ entityType: 'diagram', id: diagram.id, action: 'archive' })}/><Button size="icon" title={`Eliminar ${diagram.name}`} aria-label={`Eliminar ${diagram.name}`} icon={Trash2} onClick={() => openImpact({ entityType: 'diagram', id: diagram.id, action: 'delete' })}/></> : <Button onClick={() => diagramActions.restore.mutate(diagram.id)}>Restaurar</Button>}</div>)}
    </section>
    <div className="mt-6 flex flex-wrap gap-2"><input aria-label="Nombre de carpeta" className="rounded border border-border bg-background px-2" value={folderName} onChange={event => setFolderName(event.target.value)} /><Button variant="outline" onClick={async () => { if (folderName.trim()) { await actions.createFolder.mutateAsync(folderName); setFolderName(''); } }}>Crear carpeta</Button><Button variant="primary" onClick={() => setShowLibrary(value => !value)}>Añadir desde Biblioteca</Button></div>
