...
function Harness() { const uploads = useCanvasUploadBatches(); return <><button onClick={() => uploads.addFiles([new File(['a'], 'one.txt', { type: 'text/plain' }), new File(['b'], 'two.txt', { type: 'text/plain' })], { x: 50, y: 50 })}>Iniciar</button><CanvasUploadTray batches={uploads.batches} retry={uploads.retry} undo={uploads.undo} /></>; }
